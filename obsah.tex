...
It checks whether a given error is an spurious or not.
...
forward trace in the backward order and executing the reverse
instructions.
...
symbolic states, which is needed by the algorithm for forest automata intersection.
...
The are two possible explainations of the the worse performance of Forester with VATA
...
perform the advanced optimization between Forester and VATA, as oposed to
...
the programs on which the analysis done by Forester is time demanding.
...
the slowdown is not as significant to preveil
...
\begin{table}[bt]
	...
	Table shows the number of the programs which correctness Forester was not
	verfied before and the number of the newly found errors
...
The most of the new errors are detected in the HeapManipulation category of SV-COMP'15
benchmark, where $9$ new errors have been detected.
We also proved $2$ programs from the LDV project to be correct.
In the MemorySafety category, there is less found errors and verified programs
but it is partially caused by the fact that this category contains less test cases
then the HeapMnipulation category.
...
because it does not support used C language constructions
...
when a garbage is created but a different property is verfied.
