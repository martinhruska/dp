...
Class FA has a data member representing vector of tree automata $(t_1 \ldots t_n)$.
It also contains references to an automaton representing global variable and
...
The class keeps reference to abstract instruction to which symbol state corresponds.
It also has a forest automaton (instance of class \emph{FAE}) representing actual memory state and
a vector of data representing state of registers in such symbolic state.
Data are represented by structure \emph{Data}.
The structure consists data members keeping information about type of data, its size and
also value of data or information about its nested members.
...
\begin{itemize}
	...
	\item \emph{FI\_move\_reg} instructions creates a new symbolic state where
		is just a value from a register moved to another register.
	...
	\item \emph{FI\_node\_create} instructions creates a new node
		...
		register.
	\item \emph{FI\_node\_delete} instruction deletes a node in FA and
