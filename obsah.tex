...
For instance, Forester currently does not support the complete syntax of C and %TODO
...
%only at the one place (in VATA) then spending time with optimizing the implementation
...
is defined as follows: $c_1 \preceq_p c_2 \Leftrightarrow \emph{the cost of the cheapest path from }
\phi_1 \emph{ (input port) to } c_1 \emph{ is}$ $\emph{smaller}$ $\emph{ than the cost of the smallest path from } \phi_1 \emph{ to } c_2$.
