...
It is also not practicle to maintain and further develop such one purpose library.
...
\newpage
	...
	The displacement states a position of the selector in the label of a transition
	and it is given by the sum of sizes of the preceding selectors.
...
\bexmp
	...
	The first instruction loads to register $0$ a root reference to TA
...
\bexmp
	...
	is isolated.
	A~data value from this selector	is loaded to register $r$ by the instructions on line $2$ and $3$.
	The instructions on line $4$ and $5$ prepare data selector \codix{y->data} in the same way
	as the selector \codix{x->data} has been prepared.
	Finally on line $6$ the data value in register $3$ is copied to TA node pointed by
...
\bexmp
	...
	The instruction on line $1$ loads a root reference pointed by a selector
...
resulting automaton on lines $3$-$5$.
...
structure where the product states are stored to be processed later on line $4-8$.

The function creates a new TA with the product state of $lhs$ and $rhs$ as the root
and with an empty transition relation on line $8$.
...
These cases are in algorithm processed on line $16$-$18$ and $20$-$22$.
