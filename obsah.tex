...
This subsection describes implementation of forest automata in Forester.
The core of forest automata module is class \emph{FA}.
The implementation corresponds to the definitions given in \ref{ch:prel}.
Class FA has a data member representing vector of tree automata $(t_1 \cdots t_n)$.
It also contains a reference to an automaton representing global variable and a reference
the automata representing variables of current function. %TODO define ABP
...
(whether it is a general node, a data or a vector of data).
The structure also contains information related to the corresponding type
so it has either a data or some kind of box. 
...
As it was already mentioned the forest automata are further used in symbolic state representation.
An implementation of symbolic state is in class \emph{SymState}.
The class keeps reference to an abstract instruction $I$ to which symbolic state corresponds.
It also has a forest automaton $F$ (instance of class \emph{FAE}) representing actual memory state and
a set of reigsters $\regsset$ with data values in the symbolic state.
We denote $\regsset$ by $\regs$. 
Putting together information stored in class \emph{SymState},
we can define symbolic state formally as a triple $S=\symstate{F}{\regs}{I}$.
Data are represented by structure \emph{Data}.
The structure consists data members keeping information about type of data $T$, its size $S$ and
also value of data $V$ or information about its nested members $MBS$.
We can again fomally define data as $D = (T,S,V,MB)$ where
$T$ is a data type, $S \in \natnum$, $MB=(D_1 \cdots D_n)$, $D_i$ is another instance of data,
$V$ is a value of type in $T$.
$V$ could be value of integer, boolean or could contain adress to memory.
It could be also root reference $RR$ which is defined (in implementation) as $RR=(\emph{Root},\emph{Displ})$
...
Class \emph{VirtualMachine} has one data member which is a instance of class \emph{FAE}.
So the class has mainly the operations for manipulation with nodes of forest automata or
with references stored in FAE.
...
and removing symbolic states to a queue.
When a symbolic state is taken from the queue an instruction contained in
the symbolic state is executed.
Some instructions during its execution appends a new symbolic state to the queue.
Now we give description of semantics of the Forester microcode instructions.
In the following text the source and destination registers are paramters of
a instruction.
We use $r_s \in \regs$ to denote the source register and $r_d \in \regs$ the destination register.
The registers are parameters of some instructions.
We use $\regssub{r}{r'}$ to denote a set of registers which has same registers as $\regs$
excluding $r$ that has value $r'$.
A symbol $I_n$ denotes a next instruction following an instruction actually executed.
We define semantics of each instruction formally using a function 
$f: \instrset \times \symset \rightarrow \symset$
where $\symset$ is a set of all symbolic states and $\instrset$ is a set of all instruction.
When it is clear for which instruction is $f$ defined
we ommit the first parameter of $f$.
...
\begin{itemize}
	...
	\item \emph{FI\_set\_greg} loads a value from a local register to
		a global register.
	\item \emph{FI\_get\_ABP} loads a reference to a FA representing
		a memory pointed by local variables.
	\item \emph{FI\_get\_GLOB} loads a reference to a FA representing
		a memory of local function to a local register.
	\item \emph{FI\_load} loads a value from a FA pointed
		by a value in source register to a destination register.
		The value is value of $i$-the successor of the root pointed
		by source register value, where $i$ is a value of offset.
		The offset is paramter of FI\_load instruction.
	...
	\item \emph{FI\_store} instruction stores a value from a FA pointed by source
		register to a location in a FA pointed by destination register.
	...
	\item \emph{FI\_alloc} instruction creates a new instance of \emph{Data} structure
	...
	\item \emph{FI\_node\_create} instructions creates a new node
		...
		A reference to new TA is stored to a destination register.
	...
	\item \emph{FI\_assert} instruction checks whether a value
		...
		Both register and constant are parameters of this instruction.
	\item \emph{FI\_build\_struct} instruction creates a structure from a
		...
		The source registers are defined by a index of start register and
