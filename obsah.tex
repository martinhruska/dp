...
has really the rank $1$ and the one sucessing state is $s_{next}$.
...
\begin{itemize}
		\item We merge $TA$ of $F$ into a form where the roots of forests accepted by $F$ are cut-points only.
			...
			substitued by $q \rightarrow  \overline{a} (q_1 \cdots q_a \cdots q_n) \in \Delta_B$,
...
$\langlen{M}{q}$ is a language containg sub-trees obtained from trees of $L(M,q)$
by their restriction upto height $\leq n$.
...
In \cite{cegar}, the \emph{counterexample-guided abstaction refinement} (CEGAR)
