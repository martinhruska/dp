...
However, Forester does not use predicate abstraction but \emph{heigh abstraction} now because it is not possible to use it when the backward run is not implemented.
...
The verfication procedure based on FA is covered in Chapter \ref{ch:fav}.
...
The design of backward run for FA base verfication is given in Chapter \ref{ch:backward} and its implementation is documented
...
It was mentioned that there are currently implemented two representation of TA and these will be described in the followin subsection.
...
The advantage of this implementation is its simplicity and optimization of operation hardly exploited by Forester what makes verification run faste.
...
These arguments lead us to implement a version of Forester using the VATA library what will be described in Section \ref{ch:fova}.
...
We originally supposed that it will be possible to keep the original TA module along the VATA library adapter
...
The theory of forest automata and related theory tree automata has been studied and described in this thesis and the verification procedure
...
The knowledge about Forester and related verification procedure gained during the work on the semestral project will be further employed for design
