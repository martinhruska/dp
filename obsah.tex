...
Another approach is \emph{abstract interpretation} where the analysis is performed by
...
E.g. Forester currently does not support complete C language syntax and it is not also yet possible to check whether a found error
is real or spurious.
...
More formally, in-degree is defined as $idg(v') = |\{v \mapsto (a, v_1 \cdots v_n) \,|\, v \mapsto (a, v_1 \cdots v_n) \emph{ is an edge such that } \exists i \in \{1,\ldots,n\}: v' = v_i\}|$.
...
The properties make possible to represent an io-forest in unique way what makes possible
to manipulate it in deterministic way.
...
The minimal io-forest is unique up-tp to permutations of $t_1,\ldots,t_n$.
...
run $\rho$ over $t$ to $q$.
Finally, we can define the language of TA.
...
The second one is the graph language obtained by connecting of the io-forests from forest language to io-graphs.
...
The main part of the adapter design patter is in our case newly implemented class \emph{VATAAdapter} playing role of Adaptor.
