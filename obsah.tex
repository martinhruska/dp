...
It is not an easy task to guarantee that a software has the specified properties.
...
Finding some of the errors during testing does not imply the elimination of all of them.
...
This strenghs the \emph{maintainability} of Forester.
Having the special encapsulated module for tree automata with a clear interface
brings also the \emph{modularity} by descreasing the dependencies on the internals of
...
Implementing predicate abstraction enables the analysis of even more complex data structures like red-black trees.
...
the indicies of the output ports.
...
To be able to define trully canonicity representation of the forest $f$
...
We can symmetrically define also bottom-up tree automata as quadruple $B = (Q, \Sigma, \Delta, F)$ where
\begin{itemize}
...
The semantics of TA is following.
...
The \emph{language} of $A$ is defined as $L(A) = \bigcup_{q\in R} L(q)$.

\bexmp
Let us have a tree automaton $A=(Q,\Sigma,\Delta, R)$
...
One of the most important operation over forest automata is checking
...
Checking inclusion of languages of forest automata respecting canonicity can be done \emph{component-wise},
...
Since we already extended the notion of labels to the structured ones so
we also define tree automata over such labels.
...
Forest automata of level 1 can have only the structured labels in alphabet but not forest automata.
A set $\Gamma_1$ consists of all forest automata over $2^\Gamma$ so each \emph{forest automata of level 1} belongs to $\Gamma_1$.
...
Informally, the method removes a sub-edge and matches its origin
...
These programs can change the shape of a heap (and so reach another heap configuration)
...
unbounded, the number of the potentionally reachable heap configurations is infinite.
However, it is possible to represent this infinite state space by forest automata in a finite way.
...
like a dereference of uninitialized pointer can happen.
...
we provide the overview of symbolic execution that employs the framework of abstract interpretation.
...
However, there is still the problem of thte state explosion that often makes
...
It is very lightweighted and containinc the optimized
...
The first one is the easier maintainance of the library where
...
The first performance tests were performed over the test set
...
The implementation of backward run confirms $6$ real errors in the programs
