...
Consider the following data structure that is an implementation of singly-linked list.
	...
	$r0, r1, r2$ are operands such that $r0$ is a destination register,
	$r1$ and $r2$ are the source registers.
	It is also possible that there is a constant instead of a register
	as an operand of the instruction.
	When $[r0+c]$ is used as an operand that the instruction dereferences a root reference in register $0$.
	It means that the instruction access directly a selector with displacement $c$ in a label in the transition where
	the referenced root is the parent state.
	Note that when it is not stated otherwise it is reffered to a selector in a label of a transition
	where the root of the transition is the referenced TA by a specified register.
...
\bexmp
	...
	Finally lines $4$ a $5$ performs adding root reference pointing to $t$ to a FA that represents
...
\bexmp
	...
	The root reference is chosen by the selector with displacement $12$ in a label in TA referenced by register $ABP$.
...
\bexmp
	...
	The instruction at line $1$ loads a root reference pointed by a selector related to a pointer $x$
	to register $1$.
...
The examples should bring some intuition and
...
\begin{itemize}
	\item {\tt acc\_sel} isolates the $i$-th selector of a label
	...
	\item {\tt acc\_set,acc\_all} instruction does the same thing as the previous one
		but for a set of selectors or for all selectors of a given root of TA.
	
	\item {\tt node\_create} instructions creates a new node
	...
	\item {\tt node\_free} instruction deletes a node in FA and
	...
	\item {\tt store} instruction stores a value from the source register
	...
	\item {\tt loads, stores} instructions do same as the previous
		one but manipulates the structures.
		
		Formal definition is also same but the result in the register $r_d$ (in case of {\tt loads})
		and or in the corresponding (in case of {\tt stores}) has a different typed
...
\begin{itemize}
	...
	\item {\tt alloc} instruction creates a new instance of structure {\tt Data} $\ddata$
	...
	\item {\tt load\_cst} instruction loads a constant to a register.
	...
	\item {\tt move\_reg} instructions creates a new symbolic state where
		a value from $r_s$ is moved to register $r_d$.
		
		$f(\stdsym) = \symstate{F}{\regssub{r_d}{r_s}}{I_n}$.
	
	\item {\tt bnot, inot} instructions negate a value in a register and
	...
	\item {\tt move\_reg\_offs} instruction accesses a tree automaton pointed
	...
	\item {\tt move\_reg\_inc} instruction does same thing as the previous one but
	...
	\item {\tt get\_greg} loads a value from a global register to
		a local register.
		
		$f(\stdsym) = \symstate{F}{\regssub{r_{ld}}{r_{gs}}}{I_n}$.
	
	\item {\tt set\_greg} loads a value from a local register to
		a global register.
		
		$f(\stdsym) = \symstate{F}{\regssub{r_{gd}}{r_{ls}}}{I_n}$.
	
	\item {\tt get\_ABP} loads a root reference to a FA representing
	...
	\item {\tt get\_GLOB} loads a root reference to a FA representing
	...
	\item {\tt load} loads a value from a FA pointed
	...
	\item {\tt load\_ABP, load\_GLOB} same as the previous instruction
	...
	\item {\tt push\_greg} instruction creates a new global register
	...
	\item {\tt pop\_greg} instruction takes a value in the last created
	...
	\item {\tt cond} instruction represents a condition in the original code.
		...
		the content of {\tt cond} instruction symbolic state register.
	...
	\item {\tt iadd, imull} instructions performs integer
	...
	\item {\tt eq, neq, ge, gt, le, lt} instructions
	...
	\item {\tt build\_struct} instruction creates a structure from a
	...
	\item {\tt assert} instruction checks whether a value
