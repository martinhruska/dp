...
However, the testing language inclusion with the same algorithm used for the non-hiearchical FA
...
$F$ is a forest automata of higher level respecting canonicity and
...
Let us describe the notion of abstract transformers.
...
such that $\bigcup_{F' \in S'} \llbracket F' \rrbracket = \{\concrop \mid g_{sf} \in \llbracket F \rrbracket \wedge F \in S~\}$.
...
an FA for initial heap configuration representing an io-graph
...
in transitions what make forest automaton able to represent infinite state space.
...
This it could led to a detection of a spurious counterexample which is not present
...
Predicate abstraction is refined by predicates learnt from backward run
...
However, it is impractical to maintain a special TA library inside of Forester.
This library has the strong dependencies with the rest of codes.
The other classes depend to the library data types,
manipulates its data types and so on.
...
But the development in algorithms for TA, especially for checking inclusion of TA
languages, goes on and an implementation of the new algorithms is necessary
for greater efficiency.
A~new algorithm can also bring greater efficiency than
...
Let us describe the high-level conceptual design of Forester (which is shown in Figure~\ref{fig:fa_design})
...
The notion of the modules used in the following text is rather an abstract one, serving to reader basic summary of the Forester design.
...
code of GCC in representation provided by Code Listener to its own microcode.
...
for manipulation FA during the verification procedure.
...
because efficiency of inclusion checking is crucial for the performance.
...
$\mathit{MB}=(D_1 \cdots D_n)$ represents nested data when $D$ represents a data structure.
...
\bexmp
	...
	The second instruction performs isolation of the selector with displacement $0$ in the TA referenced by register $0$.
	...
	the new cut-point will be created created at this node
	so the special TA representing the heap reachable from \codix{y} will be needed.
...
Each instruction is first described informally and
...
\begin{itemize}
	...
	\item {\tt get\_$\abp$($r_d$,$\glb$, $o$)} loads a root reference from the register $\abp$ to
	...
	\item {\tt get\_$\glb$($r_d$,$\abp$,$o$)} loads a root reference from the register $\abp$
...
There are also the othe related classes, e.g.,
...
Orignally, we were planning keep the original TA module along VATA adapter
...
together wtih the abstract transformers at some program locations,
...
the $i$-th abstract tranformer $\abstr^{i}$ in the forward run
...
A~found error configuration is shown as a red area in the last oval.
...
The function $\mathit{intersect}$ takes as the input parameter these states together with
...
This comes from nondeterminism causing that a referencing state $r_\A$ from $\A$
forms more then one product state with the referencing states $r_\B$ from $\B$.
FA $\F_{\CC}$ defines by their product states the several bijections between
...
can led to a generation of a graph where one cut-point appers more then once
...
no occurence such a product state is in the other TA.
