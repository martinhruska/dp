...
$\mathit{cnt}$ and the data structure for the resulting automaton $F$ on lines $1$-$3$.
Then it iterates over all port indicies of the input FA
and it calls the function $\mathit{intersect}$ over the TA at position in
the~input FA given by a port index.

The function $\mathit{intersect}$ takes as the input parameter these TA
and one of their states.
We use $\mathit{lhs}$ to denote one of the inputs and
$\mathit{lhs.aut}$ to denote TA itself and $\lstate$ to denote its state.
The same notation is used for the second input $\mathit{rhs}$.
...
the stack $\mathit{workset}$ is initialized to serve such as an auxiliary data
structure where the product states are stored to be processed later on lines $4-8$.

The function $\mathit{intersect}$ creates a new TA with the product state $(\lstate, \rstate)$
...
\begin{enumerate}
	\item The both $r$ and $s$ are referencing states.
		The function $\mathit{intersect}$ is called recursively on the~referenced
		  ...
		  Finally a transition with the product state $(r,s)$ at left-handed side
...
to the resulting FA and returns the index of this TA in FA what is done on lines $28$-$29$.
...
The useless states are removed from FA.
We continue for each FA separately.

It is also necessary to care about the case when a root reference $r$ of one FA
...
the $n$ variants of the FA $F=(T_1 \cdots T_n, \port)$ are created.
For each FA $F$ of the $n$ variants,
it holds that if exists a normal state $s$ such that $(r,s)$ is in TA $T_i$, $1 \leq i \leq n$
then there is not a normal state $s'$ such that $(r,s')$ is in $T_i$
and $r,s$ is not in any other TA of $F$.
...
An overview of the backward run was given in the beginning of this chapter.
We describe now design of the backward run over symbolic state in Forester.

It was mentioned that Forester implements abstract
transformers as instructions of its microcode.
The input C program is translated to the list $T=\isfwdp{1}, \isfwdp{2},\ldots, \isfwdp{n+1}$
of the microcode instructions.
An instruction has a symbolic state as its input and
returns a~new symbolic state.
The part of a symbolic state contains also forest automata representing
the reachable heap configurations in the symbolic state.
...
We use $\ftrace$ to denote the trace and we call it the \emph{forward trace}.
...
and an execution of the \emph{reverse instruction} $\isbwdp{j}$ to each $\isfwdp{j}$
over $\sbwdp{j}$ obtaining $\sbwdp{j-1}$ where $0 < j \leq i$.
We obtain the \emph{backward trace} $\btraceseq{i}$ by the backward run.
We denote the trace by $\btrace$.
Note that a reverse instruction $\isbwd$ corresponds to a
reverse abstract transformer $\rabstr$ that is
the reversion of an abstract transformer $\abstr$
implemented by an instruction~$\isfwd$.
...
\begin{itemize}
	\item {\tt FI\_node\_create($r_s$, $r_d$)}
		...
		The new symbolic state has the same values in the registers as the state $\sbwd$ with
		the exception of the register $r_d$.
		This register is substituted by the value $r'_{d}$ of $\sfwd$.
	...
	\item {\tt FI\_node\_free}
		...
		$\grev(\bstdsym) = \symstate{F'}
			...
			$T$ is TA referenced by $RR$ in the register $r_d$ of $\sfwd$
	...
	\item {\tt FI\_push\_greg($r_g$)}
		...
		last global register from is removed the new state (what inverts push operation).
...
So reversing them consists of creating a symbolic state $S'$ with
...
to perform the prdicate abstraction over forest automata component wise.
...
The predicates are languages represented by tree automata.
...
Note that one TA can represent the several predicates.
We create the product automata $A \times \predauti{1}, \ldots, A~\times \predauti{m}$
by the algorithm for the bottom-up intersection.
...
because $\lang(F_F) \cap \lang(F_B) = \emptyset$ where FA $F_F$ is from the forward run
and FA $F_B$ is from the backward run.
...
The abstraction is choiced by setting value of macro
...
However, employinh the algorithm for reduction of tree automata in VATA would be
needed to be done.
...
Forester was published in proceedings of TACAS 2015 \cite{svcomp15}.
