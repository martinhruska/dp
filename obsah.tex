...
In last few decades importance of computers in our everyday life has largely increased.
...
There is also wide usage of computers in more critical applications like autopilot in an airplane
...
on running a program in the different contexts and under the different inputs
and checking whether programs behavior and outputs are expected one.
This method can satisfy many of requirements for software quality and often cover great space of programs behaviors
...
pointers are dereferences (no invalid dereference), whether all allocated memory on a heap is also freed (no memory leaks)
a program execution or whether there is not dislocated pointer which has not have assigned memory (no invalid free).
There are different approaches to this kind of static analysis with the different advantages.
For example approach based on \emph{separation logic} brings scalability to a verification procedure.
On the other side, automata based approach, concretely \emph{abstract tree regular model checking}, is
superior in flexibility.
However, this thesis will be focused to a verification procedure employing concept of Forest automata (FA) which tries
to synthesize benefits of the mentioned approaches.

Forest automata has been introduce in \cite{forester11,forester12} and they are extension of finite automata or more precisely finite tree automata.
They are used as abstract domain in this verification procedure.
Basically, the verification procedure based on FA is performing symbolic execution of a verified program and
...
The error could be spurious because of used to high abstraction over FA domain but the information get by backward could be used for refinement of \emph{predicate abstraction}
...
A structure of this thesis will be following.
...
Chapter \ref{ch:impl} provides description of implementation and finally Chapter \ref{ch:eval} is about evaluation.
...
We will in-degree of a node $v \in V$ by $idg_g(v)$ and we will again omit subscript $g$ whenever it is possible.
...
is a sequence of port indices, where $\pi_1$ is the input port index and $\pi_2 \ldots \pi_{|\pi|}$ is a sequence
...
The io-forest $f_c$ is \emph{canonical} iff it is minimal and the trees $t_1,\ldots, t_n$ are ordered by $\preceq_p$ and $\times f$ is accessible.
...
We will denote $i$-th sub-edge of $e$ in $g$ by $e\langle i\rangle = v \mapsto (a_i,v_k \cdots v_l)$ where $i \in \{1,\ldots,m\}$ or
...
\begin{itemize}
	...
	we will again denote the $i$-th rule term of sequence by $d\langle i\rangle$ where $i \in \{1,\ldots,m\}$.
...
Informally, an forest automaton of higher level has another forest automata (of lower lever) as symbols on its edges.
...
We will define operation \emph{sub-edge replacement} which help us to define semantics of forest automata of higher level languages.
Informally, the sub-edge replacement removes one sub-edge and matches its origin and outputs with a new graph serving like a substitution of sub-edge.
...
When we come back definition of canonicity respecting FA we will find that it is applicable also for FA of higher level.
...
of using FA in shape analysis which checks whether it is not possible for the program to reach an error like double free or
...
A single heap configuration is a io-graph $g_{st}$ over the ranked alphabet of the structured labels $2^\Gamma$
...
Consider following example taken from \cite{techrep}.
...
which models the heap configurations.
...
The sequence of \emph{abstract transformers} related to the program statements is then iteratively applied to abstract configuration.
...
This function models semantics of \texttt{op} in concrete domain in such a way that $f_{\texttt{op}}(g_{st})$
...
\begin{enumerate}
		...
		\item At junctions corresponding to the loops the union is followed by \emph{widening}
			...
			It is necessary to transform the FA to canonicity respecting form before inclusion checking.
\end{enumerate}
...
\begin{enumerate}
		...
		\item Abstraction -- It is based on framework if \emph{abstract regular (tree) model checking} \cite{artmc}.
...
Since it is possible for FA to use other FA as symbols we can fold recurring sub-graphs to the boxes further used as symbols
...
The boxes can be learn during analysing of a program automatically what was proposed in \cite{forester13} or
...
but it also implements state-of-the-art algorithms \cite{tacas10} for language inclusion checking which efficiency
...
create automaton in the chosen encoding directly using API provided by VATA).
...
and of course needed operations and can employ already implemented parses and serializers of \vata.
...
a second level of the hash tables hierarchy (\emph{transition cluster}) where are store all symbols which
...
\input{fig/explicit.tikz}
	\caption{Explicit representation of NTA by \vata. Figure is taken from \cite{libvata}}.
	\label{fig:explnta}
\end{center}
\end{figure}
...
Because semi-symbolic encoding and MTBDD are not in the aim of this thesis the will no be described in detail
but it is possible to find such kind of description in \cite{mt:lengal}.
...
The former one symmetrically maps each n-tuple $(q_1,\ldots,q_n)$ of a NFA using MTBDD to the sets of states
...
To get high level overview of verification process performed by Forester please see Figure \ref{fig:fa_exec}. 
...
When symbolic execution is over Forester checks whether there is no left garbage (of course, garbage is gradually checked also during the symbolic execution)
...
between its modules will be given now.
...
One module could be e.g. set of closely related classes with a similar purpose.
As it was mentioned above Code Listener representation of GCC intermediate code is mainly used by Forester \emph{Compiler} module.
...
These operation could be understood like another module \emph{Operation over Forest Automata}.
...
On the other side, it is not easy to maintain such a optimized implementation when one consider that there are still progress in field of design of efficient algorithms
...
Please note, that is just conceptual high level view of design Forester.
...
These arguments lead us to implement a version of Forester using the VATA library what will be described in Section \ref{sec:fova}.
...
it is currently the one that support the most of needed operations over TA and it is also more efficient than
...
Then it is would be possible to apply design pattern \emph{adapter} \cite{gamma95} (described in Section \ref{sec:adapter} to create
...
It is also creates one place (particularly adapter class) connecting Forester and VATA instead of
...
to refactore implementation before it would be possible to create adapter class for \vata API.
...
We decided to used the implementation approach to Adapter preferring composition over inheritance
...
We initially supposed that it will be possible to keep the original TA module with VATA library
...
and (b) extend verification procedure based on forest automata with backward run for detection of the spurious errors find out in a analysed program.
The theory of forest automata and related theory tree automata has been study and described in this thesis and the verification procedure
based on forest automata has been also explored to fulfill thesis goal.
...
The knowledge about Forester and related verification procedure gained during the work on the semestral project will be further used for design
