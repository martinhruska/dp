...
\bexmp
	...
	The first instruction stores a reference to the $x$ pointer selector to the register $0$.
	...
	cut-point is created when pointer $y$ also points to this memory.
	...
	a TA node pointed by pointer $y$ selector.
...
\bexmp
	...
	of the pointer $y$.
	The second one isolates all selectors of the TA referenced by register $0$
	and then the third instruction removes node pointed by register $0$
	and finally invalidates all references to it.
\eexmp
...
\bexmp
	...
	of the pointer $x$ (which is in this $4$-th selector of a label under the referenced root).
	Then the data selector $x->data$ is isolated and a data value from this selector
	is loaded to register $r$ by the instructions at line $2$ and $3$.
	The instructions at line $4$ and $5$ prepares data selector $y->data$ in the same way
	as the selector $y->data$ has been prepared.
