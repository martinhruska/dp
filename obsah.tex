...
A convertion of a data type of parameter of tree automata operation is also sometimes needed, e.g. from vector to set. 
...
Now, a more technical and algorithmical description takes a place.
...
Note that one TA can repreresent more predicates.
...
Moreover, this approach keeps the original Forester behavious
...
This chapter summarizes the exeprimental evaluation of the results of this work.
...
without the compiler optimizations are in Table \ref{tab:vatadef}.
...
with a lot of states would be inneficient.
...
and maintability coming from the separate library
...
The implementation of backward run confirms $10$ real errors in the programs
...
The main problem complicating the successfull analysis of
more programs is again imaturity of the Forester tool.
...
Forester outperforms Predator in verication of a red black lists.
...
It needs to be implemeted more efficiently to get the better
...
to analyze before.
Morever, Forester is to our best knowledge the only one sound tool
able to verify the red black lists (and similiar data structures)
