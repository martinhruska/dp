...
It proves the program safty in a standard mathematical way -- starting from the axioms and using the inference rules to
...
It is cable to verify non-trivial data structures like skip-lists of the~second and the~third level.
However, the current implementation is far from perfectness.
...
because predicate abstraction needs backward run for creating the new predicates.
...
Chapter \ref{ch:tools} provides description of \vata\ and Forester tool
...
for FA base verification is given in Chapter \ref{ch:backward}.
...
This chapter provides the theoretical foundations of this thesis.
...
\emph{In-degree} of a node $v' \in V$ in the graph $g$ is the overall number of
its occurrences in $g(v)$ for any $v \in V$.
...
$idg(v') = |\{v \mapsto (a, v_1 \cdots v_n) \,|\, v \mapsto (a, v_1 \cdots v_n)
...
an edge $v_1 \mapsto (a,(v_2,v_3))$.
There are many paths but to illustrate the concept of path we give
...
That enables manipulating with io-forest deterministically.
...
in alphabe of FA $F'$ then $F'$ is FA of the firest level.
Then the hierarchy of FA is created by nesting the FA of lower level
to the labels of FA of higher lavel.
...
The verification starts from an initial abstract configuration that consists an FA for initial heap configuration representing io-graph
...
\begin{itemize}
		...
		\item We obtain form of $F$ in which the roots of trees of forests from language of the transformed
			...
			\begin{itemize}
				\item The root of $t_i$ is the $j$-th cut-point in the canonical ordering of the cutpoints
...
Morever, no refinement has been implemented in the Forester tool yet.
...
A deeper description of conceptual design of Forester (which is shown in Figure~\ref{fig:fa_design})
...
Consider the following data structure that is an implementation of singly-linked list.
	...
	the referenced root at lef-handed side if it is not stated otherwise.
...
\bexmp
	...
	Firstly the size of the new allocated data (in this case the size is $4$) is stored to the register $0$ at line~$1$.
	...
	In this case a selector related to the pointer x has displacemnt $12$ in the label of TA
...
\bexmp
	...
	This creates of a~new TA if \codix{x->next} points to an allocated memory (because a new
...
We use $\regssub{r}{r'}$ to denote a~set of the registers which is the same as
the set $\regs$ but register $r$ is substituted by register $r'$.
A symbol $I_n$ denotes the next instruction following an instruction actually executed.
...
The instructions are divided to the two parts.
The first part contains the instructions modifying forest automata.
The second part consists the rest of the other instructions.
...
The implementation of backward run confirms the $10$ real errors in the programs
...
This section provides evaluation of the Forester using predicate abstraction.
...
Now the particular programs analysis is given
comparing performance of Forester and Predator.
...
Forester outperformed Predator in verication of a red black list,
...
So for examaple, a distance between head of the list and a 
node having integer $1$ as a data member is smaller than
the distance between a node having integer $2$ as a data member.
In case of verification of correct program, Predator outperforms Forester
because Forester found few spurious counterexamples
...
but the times are very similiar.
...
Forester is able to verify correctness of such a structure
...
freeing the list two nodes at time (what is safe because
...
unable to estabilish a predicate set avoiding this error.
It created gradually predicate defining that list would not
be $3, 5, \cdots$ long but was unable to generalize
the predicates to all odd numbers.

It is necessary admit that predicate abstraction
is not fully complete.
It needs to be implemeted more efficiently to get better
performance on the hard cases.
The new predicates are not also created in the most
precise way so some state may be not abstract
although it would be theoritically possible.
Then predicate abstraction often causes an internal
...
and described in this thesis and the verification procedure
...
Then the adapter pattern was used to create interace between Forester and VATA.
...
Then backward run over symbolic states was designed for Forest automata based shape anlysis
an the design was implemented to tool Fotester.
...
been yet proposed.
Predicate abstraction over forest automata was also designed empoying the predicates
created by backward run.
Predicate abstraction employs bottom up tree automata in the VATA
...
to analyze before.
Morever, Forester is the only one sound tool able to verify red black lists (and similiar data structures)
in fully automatical way after the extension with predicate abstraction and backward run.
...
or an analysis of a program lead to an internal verifier error.
Predicate abstraction is not also yet fine-tuned.
It needs to further improvements for better performance
and also a conceptual extension that makes it able to continue also
when a garbage is created (and for example an absence of invalid
dereferences is checked in a program is verified).
Then there are also conceptual extensions of the theory.
...
data structures like singly-link lists of even length is
another challange.
