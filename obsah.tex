...
Firstly, we introduce some examples how C statements are trasnlated
...
Consider the following data structure that is an implementation of singly-linked list.
	...
	When $[r_0+c]$ is used as an operand then the instruction dereferences a root reference in register $0$.
	It means that the instruction access directly a selector with the displacement $c$ in a label in the transition where
	...
	$GLOB$ and $ABP$ global registers such that $GLOB$ register contains reference to a FA representing
