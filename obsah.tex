...
Not only the allocated memory on the heap is modelled by forest automata during the verification procedure but also
...
a function $\alpha: Q \rightarrow Q/\eqrel{M}$ such that $\alpha(q) = \eqclass{q}{M}$
...
such that $\forall q_1,q_2 \in Q: q_1 \eqrel{M} q_2 \Leftrightarrow \langlen{M}{q_1} = \langlen{M}{q_2}$
where $\langlen{M}{q}$ $=$ $\{ w \in \Sigma^n \,|\, |w| \leq n \wedge wv \in \lang{M}{q} \wedge |v| \geq 0\}$
...
of the function $\grev: \grevinter$ which should have the reverse semantics to the function $f$ from Section \ref{subsec:microinstr}.
...
\begin{itemize}
	...
	\item {\tt FI\_node\_create($r_s$, $r_d$)}
		...
		the value of the register $r_d$ is subtitued by its value $r'_{d}$ in $\sfwd$
		because the register $r_d$ in $\sbwd$ contains reference to a TA $t$
		representing heap pointed by the dealocated pointer.
