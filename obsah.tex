...
One of types is root reference which values are defined as $RR=(\emph{Root},\emph{Displ})$
...
A list of microcode instructions used in Forester with description
...
\begin{itemize}
	...
	\item \emph{FI\_acc\_sel} isolates the $i-th$ selector which is a given root of TA.
		The root of TA is defined by destination register.
		
		$f(\stdsym) = \symstate{F'}{\regs}{I_n}$
		where $F'$ is a new FA obtained from $F$ by isolating
		the $i-th$ selector of root of TA $t$.
		The register $r_d$ contains root reference $RR=(Root, Displ)$
		such that $Root$ points to TA $t$ and $i=Displ+o$ where $o$ is a offset which is a parameter
		of this instruction.
	...
	\item \emph{FI\_move\_reg\_offs} instruction access a reference to a tree automaton
		...
		$f(\stdsym) = \symstate{F}{\regssub{r_d}{(\lfunc{d}{d.RR.Displ = d.RR.Displ + o})(r_s)}}{I_n}$
	...
	\item \emph{FI\_move\_reg\_inc} instruction do same thing as the previous one but
		...
		$f(\stdsym) = \symstate{F}{\regssub{r_d}{(\lfunc{d,i}{d.RR.Displ = d.RR.Displ + i})(r_{s1}, r_{s2})}}{I_n}$
	...
	\item \emph{FI\_get\_ABP} loads a root reference to a FA representing
		...
		$f(\stdsym) = \symstate{F}{\regssub{r_{d}}{
	...
	\item \emph{FI\_load} loads a value from a FA pointed
		...
		$f(\stdsym) = \symstate{F}
			{\regssub{r_{d}}{
				\lfuncp{d}{S_{F[i]}^{displ}}{r_{s}}
			}}
			{I_n}$
			where $F[i]$ is a TA of F referenced by
			root reference $\rref$ stored in $r_s$ register,
