...
\projectinfo{
  ...
  abstract.cs={
	  ...
	  Lesní automaty jsou založeny na stromových automatech a Forester obsahuje jejich jednoduchou implementaci.
	  Prvním přínosem této práce je nahrazení této implementace knihovnou VATA, která obsahuje optimalizované algoritmy
	  pro efektivní reprezentaci a manipulaci stromových automatů.
	  Verze nástroje Forester používající knihovnu VATA se zúčastnila mezinárodní soutěže SV-COMP 2015.
	  Za druhé, verifikace založená na lesních automatech je v této práci rozšířena o analýzu nalezených protipříkladů.
	  Výsledek této analýzy je možné výužít následujícími způsoby.
	  ...
	  Právě predikátová abstrakce využívající informace ze zpětného běhu je také navržena a implementována jako součást této práce.
  ...
  abstract.en={
	  ...
	  This approach represents the shapes of heap using forest automata what are tuples of tree automata.
	  Forester currently has a simple implementation of tree automata.
	  Our first contribution is replacing this implementation by a general purpose tree automata library VATA,
	  which contains the highly optimized implementations of the automata operations.
	  ...
	  The first one allows us to determine whether a found error is a real of spurious one.
	  The results of counterexample analysis is also used for creating the new predicates
	  which are used for the refinement of predicate abstraction.
	  We show that both of these techniques contribute to the better results in comparison with the early approach.
  ...
  za poskytnuté rady během konzultací.
